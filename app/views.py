...
def canvas_home():
    if "user" in session:
        with sqlconn() as sql:
            ...
            return render_template('canvas-home.html',user=user_info["username"],hide_header = request.args.get('hide_header',False,type=bool))
    return render_template('canvas-home.html',hide_header = request.args.get('hide_header',False,type=bool))

@app.route('/snake',methods = ['GET'])
def snake_js():
    return render_template('snake-game.html',hide_header = request.args.get('hide_header',False,type=bool))
...
def physics():
    return render_template('physics.html',hide_header = request.args.get('hide_header',False,type=bool))

@app.route('/minesweeper',methods = ['GET'])
def minesweeper_js():
    return render_template('minesweeper-game.html',hide_header = request.args.get('hide_header',False,type=bool))

@app.route('/games',methods = ['GET'])
def games():
    return render_template('games.html',hide_header = request.args.get('hide_header',False,type=bool))
...
def register():
    ...
    return render_template('register.html', form=form,hide_header = request.args.get('hide_header',False,type=bool))
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if len(check)>0:
            ...
            return redirect(url_for('home',hide_header = request.args.get('hide_header',False,type=bool))),200
        else: 
            flash("You are a fraud")
            return render_template('login.html', form=form,hide_header = request.args.get('hide_header',False,type=bool)),401
    return render_template('login.html', form=form,hide_header = request.args.get('hide_header',False,type=bool)),200
    ...
    if form.validate_on_submit():
        with sqlconn() as sql:
            ...
            if len(check)>0:
                ...
                return redirect(url_for('home',hide_header = request.args.get('hide_header',False,type=bool))),400
            sql.session.execute(Update.user_username({"id":session["user"],"username":escape(form.username._value())}))
            sql.session.commit()
            session.pop("username")
            flash("Success")
            return redirect(url_for('home',hide_header = request.args.get('hide_header',False,type=bool))),200
    return render_template('username_change.html',form=form,hide_header = request.args.get('hide_header',False,type=bool)),200
...
def remove_account():
    if "user" not in session:
        return redirect(url_for('home',hide_header = request.args.get('hide_header',False,type=bool)))
    form = UsernameForm()
    if form.validate_on_submit():
        with sqlconn() as sql:
...
def home():
    if "user" in session:
        with sqlconn() as sql:
            ...
            return render_template('home.html',user=user_info["username"], picture = profile_photos_dir+user_info["picture"]["profile_picture"],stats = user_scores,hide_header = request.args.get('hide_header',False,type=bool))
    return render_template('home.html',hide_header = request.args.get('hide_header',False,type=bool))
...
def terms_of_service():
    if "user" in session:
        with sqlconn() as sql:
            user_info = get_user_info(sql)
            return render_template('terms-of-service.html',user=user_info["username"],hide_header = request.args.get('hide_header',False,type=bool))
    return render_template('terms-of-service.html',hide_header = request.args.get('hide_header',False,type=bool))
...
def privacy_policy():
    if "user" in session:
        with sqlconn() as sql:
            user_info = get_user_info(sql)
            return render_template('privacy-policy.html',user=user_info["username"],hide_header = request.args.get('hide_header',False,type=bool))
    return render_template('privacy-policy.html',hide_header = request.args.get('hide_header',False,type=bool))
